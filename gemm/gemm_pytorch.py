...
def initialize(Ni, Nj, Nk, iter, device):
    ...
    #                     dtype=dtype)
    # B_ref =np.fromfunction(lambda k, j: (k * (j + 2) % Nj) / Nj, (Nk, Nj),
    #                     dtype=dtype)
    # C_ref = np.fromfunction(lambda i, j: ((i * j + 1) % Ni) / Ni, (Ni, Nj),
    #                     dtype=dtype)

    total_elapsed = 0
    for k in range(1, iter):
        start = time.perf_counter()
        result = kernel(alpha, beta, A, B, C)
    ...
    # C_ref = torch.tensor(gemm_numpy.kernel(alpha, beta, A_ref, B_ref, C_ref))
